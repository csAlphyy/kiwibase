...
netvar_t* netvar = nullptr;

void netvar_t::get( void )
{
    m_tables.clear( );

    if ( !csgo.m_client.client_dll )
        return;

    auto prop_class = csgo.m_client.get_all_classes( );
    ...
    while ( prop_class )
    {
        m_tables.push_back( prop_class->m_table );
        prop_class = prop_class->m_next;
    }
}
